...
import { CANNOT_LIKE_OWN_DORMSPACE } from "@/lib/dormspace-engagement";
...
export async function POST(req: Request) {
  ...
  if (dormspace.landlord_user_id === session.userId) {
    return fail("FORBIDDEN", CANNOT_LIKE_OWN_DORMSPACE, 403);
  }

  const { error } = await admin.from("dormspace_likes").insert({
  ...
  if (error) {
    ...
    console.error("[dormspaces/likes] insert failed", error);
    return fail("SERVER_ERROR", "Could not save like", 500);
  }
