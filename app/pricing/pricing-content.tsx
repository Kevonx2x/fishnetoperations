...
import type { SessionProfile } from "@/lib/admin-api-auth";
import type { PaymongoSubscriptionTier } from "@/lib/paymongo";
...
export function PricingContent({ session, currentTier }: PricingContentProps) {
  const isAgent = session?.role === "agent" || session?.role === "broker";
  const normalized = normalizeListingTier(currentTier);
  const [loadingTier, setLoadingTier] = useState<PaymongoSubscriptionTier | null>(null);

  async function upgrade(tier: PaymongoSubscriptionTier) {
    setLoadingTier(tier);
    try {
      ...
      <div className="px-4 py-12 md:py-16">
        <div className="mx-auto max-w-6xl">
          ...
          <p className="mx-auto mt-3 max-w-2xl text-center text-sm font-semibold text-[#2C2C2C]/55">
            ...
            showing assistants. Pay securely with PayMongo — cards and local wallets where supported.
          </p>

          <div className="mt-12 grid gap-5 lg:grid-cols-4">
            {tiers.map((tier) => {
              const isFeatured = tier.highlight === "popular";
              const isCurrent = isAgent && normalized === tier.id;
              const paidTier =
                tier.id === "pro" || tier.id === "featured" || tier.id === "broker" ? tier.id : null;
              return (
                <div
                  key={tier.id}
                ...
                >
                  ...
                  {isCurrent ? (
                    <span className="mb-2 inline-flex w-fit rounded-full bg-[#6B9E6E]/15 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-[#2C2C2C]">
                      Current plan
                    </span>
                  ) : null}
                  ...
                  <div className="mt-6">
                    {paidTier && isAgent ? (
                      <button
                        ...
                        onClick={() => void upgrade(paidTier)}
                        className="inline-flex w-full items-center justify-center rounded-full bg-[#2C2C2C] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#6B9E6E] disabled:cursor-not-allowed disabled:opacity-50"
                      >
                      ...
                      <Link
                        href="/auth/signup"
                        className="inline-flex w-full items-center justify-center rounded-full border border-[#2C2C2C]/20 bg-white px-4 py-2.5 text-sm font-bold text-[#2C2C2C] hover:bg-[#FAF8F4]"
                      >
                        Get started
                      </Link>
                    ) : tier.id === "free" && isAgent ? (
                      <Link
                        href="/dashboard/agent?tab=billing"
                        className="inline-flex w-full items-center justify-center rounded-full border border-[#2C2C2C]/20 bg-white px-4 py-2.5 text-sm font-bold text-[#2C2C2C] hover:bg-[#FAF8F4]"
                      >
                        {isCurrent ? "Current plan" : "View billing"}
                      </Link>
                    ) : (
                      <Link
                        href="/auth/signup"
