...
function BahaygoThreadHeader({ currentUserId }: { currentUserId: string }) {
...
export function AgentChatInbox({
  initialChannelId,
}: {
  ...
  return (
    <div className="bahaygo-stream-chat font-sans text-[#2C2C2C]">
      <style jsx global>{`
        .bahaygo-stream-chat {
        ...
        .bahaygo-stream-chat .str-chat__main-panel {
          background: #faf8f4;
        }
      ...
      <Chat client={client} theme="messaging light">
        <div className="flex min-h-[560px] flex-col overflow-hidden rounded-2xl border border-[#2C2C2C]/10 bg-[#FAF8F4] shadow-sm md:flex-row">
          <div className="flex max-h-[40vh] w-full flex-shrink-0 flex-col border-b border-[#2C2C2C]/10 md:max-h-none md:w-[min(100%,320px)] md:border-b-0 md:border-r">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto">
              <ChannelList
                ...
                EmptyStateIndicator={() => (
                  <p className="px-3 py-6 text-center font-sans text-sm text-[#2C2C2C]/55">No conversations yet.</p>
                )}
              />
            </div>
          </div>

          <div className="min-h-[360px] min-w-0 flex-1 bg-[#FAF8F4]">
            <Channel EmptyPlaceholder={<EmptyThread />}>
              <Window>
                <BahaygoThreadHeader currentUserId={user.id} />
                <MessageList />
                <MessageInput focus />
              </Window>
            </Channel>
...
function EmptyThread() {
  return (
    <div className="flex h-full min-h-[280px] flex-col items-center justify-center px-6 text-center">
      ...
      <p className="mt-2 font-sans text-sm text-[#2C2C2C]/55">Choose a client on the left to view messages.</p>
    </div>
  );
}
