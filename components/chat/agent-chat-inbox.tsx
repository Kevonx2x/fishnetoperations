    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 flex items-center justify-between gap-2">
          ...
          {(unread ?? 0) > 0 ? <span className="ml-2 h-2 w-2 shrink-0 rounded-full bg-[#6B9E6E]" /> : null}
        </div>
      </div>
    </button>
  );
...
function AirbnbMessage(props: MessageUIComponentProps) {
  ...
  // Agent (current user) on right (sage green), client on left (gray)
  return (
          ...
          <div className="border-b border-[#2C2C2C]/10 bg-white md:border-b-0 md:border-r">
            ...
            <div className="max-h-[calc(100vh-200px-76px)] overflow-y-auto">
              <ChannelList
                ...
                EmptyStateIndicator={() => (
                  <p className="px-4 py-10 text-center text-sm font-medium text-[#2C2C2C]/55">No conversations yet.</p>
                )}
              />
            </div>
          </div>
