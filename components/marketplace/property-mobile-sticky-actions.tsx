...
import { ArrowRight, Calendar, Loader2, MessageCircle } from "lucide-react";
...
type Props = {
  ...
  /** Editorial property page — single sage “Check availability” bar. */
...
export function PropertyMobileStickyActions({
  ...
  if (variant === "editorial") {
    return (
      <div
        className="w-full shrink-0 border-t border-[#2C2C2C]/8 bg-[#FAF8F4] shadow-[0_-4px_16px_rgba(0,0,0,0.06)]"
        role="group"
      ...
      >
        <div className="mx-auto max-w-lg px-4 py-2.5">
          <button
            ...
            className={cn(
              "inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#6B9E6E] px-4 text-[11px] font-bold uppercase tracking-[0.14em] text-white shadow-sm transition hover:bg-[#5d8a60] active:bg-[#548a58] disabled:cursor-not-allowed disabled:opacity-50",
            )}
          >
            Check availability
    ...
    >
      <div className="mx-auto flex max-w-lg gap-2.5 px-4 py-2.5">
        <button
          ...
          disabled={disabled || messageDisabled || messageBusy || !onMessageAgent}
          className={cn(
